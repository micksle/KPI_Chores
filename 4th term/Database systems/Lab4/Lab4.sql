...
PRIMARY KEY (ID),
CONSTRAINT driverCONST FOREIGN KEY (Driver) REFERENCES transportation(ID)  ON UPDATE CASCADE ON DELETE CASCADE,
CONSTRAINT expCONST FOREIGN KEY (Experience) REFERENCES transportation(ID)  ON UPDATE CASCADE ON DELETE CASCADE
);
...
INSERT INTO salary(experience) SELECT ID FROM path WHERE ID = 4;
